"""Script to create a new versioned build of RDMnetBroker.

Updates the versioned input files
Prompts the user to review before committing and tagging

This script is meant to be run by an RDMnetBroker developer on a development workstation. It
assumes that git is already configured globally and git commands can be run without further
configuration.
"""
import argparse
import os
...
PRODUCT_NAME = 'RDMnetBroker'

FILE_TEMPLATE_DIR = os.path.join('tools', 'version', 'templates')
FILE_IN_PATHS = [
    os.path.join(FILE_TEMPLATE_DIR, 'broker_version_res.h.in'),
    os.path.join(FILE_TEMPLATE_DIR, 'broker_version.h.in'),
...
TAG_MSG_TEMPLATE = PRODUCT_NAME + ' version {}'
RELEASE_TAG_MSG_TEMPLATE = PRODUCT_NAME + ' release version {}'
...
def update_version_files(repo_root, version):
    """Update files with version information in them with the new version information.
    ...
    for in_file, out_file in zip(in_file_handles, out_file_handles):
        for line in in_file.readlines():
            line = line.replace('@BROKER_VERSION_MAJOR@', str(version[0]))
            line = line.replace('@BROKER_VERSION_MINOR@', str(version[1]))
            line = line.replace('@BROKER_VERSION_PATCH@', str(version[2]))
            line = line.replace('@BROKER_VERSION_BUILD@', str(version[3]))
            line = line.replace('@BROKER_VERSION_STRING@', version_str)
            line = line.replace('@BROKER_VERSION_DATESTR@', today.strftime('%d %B %Y'))
            line = line.replace('@BROKER_VERSION_COPYRIGHT@', 'Copyright ' + str(today.year) + ' ETC Inc.')
            out_file.write(line)
        in_file.close()
        out_file.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create a new versioned build of {}'.format(PRODUCT_NAME))
